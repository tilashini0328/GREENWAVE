				...
				<?php 
					...
					if($count>=1){
						...
						while($row=mysqli_fetch_array($result)){
							...
							<input type="submit" name="select" value="Accept"/>
							<input type="button" name="del" value="Reject"/>
					...
					include ('connection.php')
						if (isset($_GET['del'])) {
							$submissionID = $_GET['del'];							
	mysqli_query($db, "DELETE FROM submission WHERE submissionID =$submissionID");

	}
	?>

					<button type="button" onclick="window.location.href = 'newSub.php';" style="background-color:lightgreen;margin-left:auto;margin-right:auto;display:block;">
